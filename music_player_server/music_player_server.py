    ...
    async def music_player_server_loop(self) -> None:
        ...
        while True:
            ...
            for playing_music in self.playlist:
                if platform.system() == "Windows":
                    playing_music_filepath = (
                        os.getcwd()
                        + "\\music_player_server\\music\\"
                        + playing_music["filename"]
                    )
                if platform.system() == "Linux":
                    playing_music_filepath = (
                        os.getcwd()
                ...
                playing_music_file_numpy_array, playing_music_file_samplerate = (
                    soundfile.read(playing_music_filepath)
                )
                ...
                while current_frame < end_frame:
                    for playing_user in self.playing_users.values():
                        if not playing_user["already_playing"]:
                            ...
                            response = {
                                ...
                                "content": {
                                    "name": playing_music["name"],
                                    "authors": playing_music["authors"],
                                    "streamed_data": playing_music_file_numpy_array[
                                        : int(playing_music_file_samplerate * 0.5)
                                    ].tolist(),
                                },
                            }
                        try:
                            await playing_user["websocket_protocol"].send(
                                json.dumps(response)
                            )
                        ...
                        response.clear()
                    current_frame = current_frame + int(
                        0.5 * playing_music_file_samplerate
                    )
