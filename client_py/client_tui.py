...
async def input_processing(websocket_protocol: websockets.WebSocketClientProtocol):
    ...
    try:
        ...
        if command[0] == '%help': # Check help
            ...
            raise NotImplementedError('Chat command for TUI Client is not implemented yet.')
            # if len(command) < 2:
...
class LoginScreen(textual.screen.Screen):
    def compose(self) -> textual.app.ComposeResult:
        yield textual.containers.Grid(
            ...
            textual.widgets.Input(placeholder = 'Login Username', id = 'login_username_input'),
            textual.widgets.Input(placeholder = 'Login Password', password = True, id = 'login_password_input'),
    ...
    async def on_button_pressed(self, event: textual.widgets.Button.Pressed) -> None:
        ...
        if event.button.id == "login_confirm_button":
            login_username = self.query_one('#login_username_input', textual.widgets.Input).value
            login_password = self.query_one('#login_password_input', textual.widgets.Input).value
...
class RegisterScreen(textual.screen.Screen):
    def compose(self) -> textual.app.ComposeResult:
        yield textual.containers.Grid(
            ...
            textual.widgets.Input(placeholder = 'Register Username', id = 'register_username_input'),
            textual.widgets.Input(placeholder = 'Register Password', password = True, id = 'register_password_input'),
            textual.widgets.Button('Cancel', variant = 'error', id = 'register_cancel_button'),
    ...
    async def on_button_pressed(self, event: textual.widgets.Button.Pressed) -> None:
        ...
        if event.button.id == "register_confirm_button":
            register_username = self.query_one('#register_username_input', textual.widgets.Input).value
            register_password = self.query_one('#register_password_input', textual.widgets.Input).value
...
class GenshinOJClient(textual.app.App):
    ...
    def compose(self) -> textual.app.ComposeResult:
        ...
        with textual.containers.Container(id = 'main-container', classes = 'aligned') as self.main_container:
    ...
    async def on_buttonLogin_pressed(self, event: textual.widgets.Button.Pressed) -> None:
        global is_logged
        self.push_screen(LoginScreen())
    
    @textual.on(textual.widgets.Button.Pressed, '#register')
    async def on_buttonRegister_pressed(self, event: textual.widgets.Button.Pressed) -> None:
