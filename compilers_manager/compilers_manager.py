...
class compilers_manager:
    def __init__(self, server_instance: server.server) -> None:
        ...
        for compiler_config in self.compilers_config['compilers']:
            if compiler_config['enabled']:
                tmp_compiler: compilers_manager.compilers.base_compiler.base_compiler = getattr(getattr(getattr(importlib.__import__(compiler_config['path']), 'compilers'), compiler_config['id']), compiler_config['id'])(unload_timeout = compiler_config['unload_timeout'])
    ...
    def get_file_path_by_filename_and_language(self, filename: str, language: str) -> str:
    ...
    def get_binary_path_by_filename_and_language(self, filename: str, language: str) -> str:
        compiler_instance: compilers_manager.compilers.base_compiler.base_compiler
        for compiler_instance in self.compilers_instance:
            if language in compiler_instance.language_bind:
                if platform.system() == 'Windows':
    ...
    def get_compile_file_command_by_filename_and_language(self, filename: str, language: str) -> str:
        compiler_instance: compilers_manager.compilers.base_compiler.base_compiler
        for compiler_instance in self.compilers_instance:
            if language in compiler_instance.language_bind:
    ...
    def get_binary_execute_command_by_filename_and_language(self, filename: str, language: str) -> str:
        compiler_instance: compilers_manager.compilers.base_compiler.base_compiler
        for compiler_instance in self.compilers_instance:
            if language in compiler_instance.language_bind:
